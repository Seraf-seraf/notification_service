...
Эти версии являются базовой матрицей разработки. Если при реализации потребуется обновление patch/minor версии из-за совместимости образов или пакетов, изменение должно быть явным: обновить этот файл, Dockerfile/compose и README. В `.env.example` остаются только переменные, которые реально интерполируются compose или передаются приложению.
